...
seen (Message (Just (NickName n _ _)) _ params)
    | ln    == lnick  = privmsg chan $ printf "%s: That's you, right there." n
    | lnick == S.nick = privmsg chan $ printf "%s: That's me." n
    | otherwise       = do
        cp <- asks pool
        qr <- runDb cp $ findNick lnick
        case qr of
                (Left _)  -> privmsg chan "Connection error."
                (Right v) -> result v
    where findNick n  = findOne (select [ "nick" =: n ] collection) { sort = [ "_id" =: (-1 :: Int) ]}
          timeAgo d m = U.relTime . round $ diffUTCTime d m
          result Nothing  = privmsg chan $ printf
          ...
          result (Just d) = do
              ...
              let chn = B.at "chan" d
              let whn = B.at "date" d
              privmsg chan $ printf "%s: %s was seen %s, %s"
